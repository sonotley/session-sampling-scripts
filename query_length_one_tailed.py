...
def make_query(id: int, mean_duration: int, duration_spread: int, session_proportion: float) -> sim.Query:
    return sim.Query(
        id=id,
...
queries = [
    make_query(id=i, mean_duration=80, duration_spread=i*10,session_proportion=0.16)
    for i in range(0, 9)
]
            ...
            observed_queries = (
                ...
                .with_columns(
                    (
                            pl.col("a")
                            + (
                                (1 - np.sqrt(  # todo: this seems slightly better without the sqrt but I can't justify why
                ...
                .with_columns(
                    (pl.col("estimate_aadj") * pl.col("weight_aadj")).alias("weighted_est_aadj2"))
            )
...
combined_summary = pl.concat(results).with_columns(
    [
        ...
        for x in ("mean_est_weighted_adj2", "mean_est_weighted_adj", "mean_est_weighted", "mean_est_unweighted", "mean_est_weighted_aadj", "mean_est_weighted_aadj2")
    ]
).with_columns(
    [
        (pl.col(x)/pl.col("true_mean")).alias(f"rel{x}")
        for x in ("error_mean_est_weighted_adj2", "error_mean_est_weighted_adj", "error_mean_est_weighted", "error_mean_est_unweighted", "error_mean_est_weighted_aadj", "error_mean_est_weighted_aadj2")
    ]
)
...
aggregated_summary = combined_summary.group_by(['QID', 'sample_period']).agg(pl.mean("mean_est_weighted_adj"))
...
sns.boxplot(
    normalized_combined_summary.filter(pl.col("variable")=="relerror_mean_est_weighted_aadj2"),
    # x="query_length_in_periods",
    x="QID",
    y="value",
    native_scale=False,
...
sns.boxplot(
    # normalized_combined_summary.filter(pl.col("variable")=="relerror_mean_est_weighted_adj"),
    normalized_combined_summary.filter(pl.col("variable").str.contains("relerror_mean_est")),
    # x="query_length_in_periods",
    x="QID",
    y="value",
    native_scale=True,
...
for i in range(1,6):
    c = cp[i-1]
    sns.histplot(all_true_queries.filter(pl.col("QID")==i), x="true_duration", ax=axes2[0,i-1], binwidth=10, binrange=(0,500), color=c)
    sns.histplot(all_observed_queries.filter(pl.col("QID") == i), x="true_duration", ax=axes2[1, i-1],
                 binwidth=10, binrange=(0,500), color=c)
    sns.histplot(all_observed_queries.filter(pl.col("QID") == i), x="estimate", ax=axes2[2, i-1],
                 binwidth=10, binrange=(0,500), color=c)
    sns.histplot(all_observed_queries.filter(pl.col("QID") == i), x="estimate", weights="weight_aadj", ax=axes2[3, i-1],
                 binwidth=10, binrange=(0,500), color=c)
    sns.histplot(all_observed_queries.filter(pl.col("QID") == i), x="estimate_aadj", weights="weight_aadj", ax=axes2[4, i - 1],
                 binwidth=10, binrange=(0,500), color=c)
